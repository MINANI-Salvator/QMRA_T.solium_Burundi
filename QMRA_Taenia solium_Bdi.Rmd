...
Tot_Population_inf <-Pop_inf_daily+Pop_inf_weekly+Pop_inf_monthly+
  Pop_inf_bimonthly+Pop_inf_quarterly+Pop_inf_semesterly+Pop_inf_annually
```

### Infection based on infection intensity/ Informal slaughter
```{r}
Prob_light_infect_inform<-(PS5*Pmeal1*Viable_cyst*undercooked)+(PS6*Pmeal1*Viable_cyst*undercooked)+(PS7*Pmeal1*Viable_cyst*undercooked)+(PS8*Pmeal1*Viable_cyst*undercooked)
Prob_moder_infect_inform<-(PS9*Pmeal2*Viable_cyst*undercooked)+(PS10*Pmeal2*Viable_cyst*undercooked)+(PS11*Pmeal2*Viable_cyst*undercooked)+(PS12*Pmeal2*Viable_cyst*undercooked)
Prob_heavy_infect_inform<-(PS13*Pmeal3*Viable_cyst*undercooked)+(PS14*Pmeal3*Viable_cyst*undercooked)+(PS15*Pmeal3*Viable_cyst*undercooked)+(PS16*Pmeal3*Viable_cyst*undercooked)
Tot_informal_infective<-Prob_light_infect_inform+Prob_moder_infect_inform+Prob_heavy_infect_inform
```

### Infection based on infection intensity/formal slaughter
```{r}
Prob_light_infect_form<-((PS21*Pmeal1*Viable_cyst)+(PS23*Pmeal1*Viable_cyst)+(PS25*Pmeal1*Viable_cyst)+(PS27*Pmeal1*Viable_cyst)+(PS49*Pmeal1*Viable_cyst)+(PS51*Pmeal1*Viable_cyst)+(PS53*Pmeal1*Viable_cyst)+(PS55*Pmeal1*Viable_cyst)+(PS77*Pmeal1*Viable_cyst)+(PS79*Pmeal1*Viable_cyst)+(PS81*Pmeal1*Viable_cyst)+(PS83*Pmeal1*Viable_cyst)+(PS105*Pmeal1*Viable_cyst)+(PS107*Pmeal1*Viable_cyst)+(PS109*Pmeal1*Viable_cyst)+(PS111*Pmeal1*Viable_cyst))*undercooked

Prob_moder_infect_form<-((PS29*Pmeal2*Viable_cyst)+(PS31*Pmeal2*Viable_cyst)+(PS33*Pmeal2*Viable_cyst)+(PS35*Pmeal2*Viable_cyst)+(PS57*Pmeal2*Viable_cyst)+(PS59*Pmeal2*Viable_cyst)+(PS61*Pmeal2*Viable_cyst)+(PS63*Pmeal2*Viable_cyst)+(PS85*Pmeal2*Viable_cyst)+(PS87*Pmeal2*Viable_cyst)+(PS89*Pmeal2*Viable_cyst)+(PS91*Pmeal2*Viable_cyst)+(PS113*Pmeal2*Viable_cyst)+(PS115*Pmeal2*Viable_cyst)+(PS117*Pmeal2*Viable_cyst)+(PS119*Pmeal2*Viable_cyst))*undercooked

Prob_heavy_infect_form<-((PS37*Pmeal3*Viable_cyst)+(PS39*Pmeal3*Viable_cyst)+(PS41*Pmeal3*Viable_cyst)+(PS43*Pmeal3*Viable_cyst)+(PS65*Pmeal3*Viable_cyst)+(PS67*Pmeal3*Viable_cyst)+(PS69*Pmeal3*Viable_cyst)+(PS71*Pmeal3*Viable_cyst)+(PS93*Pmeal3*Viable_cyst)+(PS95*Pmeal3*Viable_cyst)+(PS97*Pmeal3*Viable_cyst)+(PS99*Pmeal3*Viable_cyst)+(PS121*Pmeal3*Viable_cyst)+(PS123*Pmeal3*Viable_cyst)+(PS125*Pmeal3*Viable_cyst)+(PS127*Pmeal3*Viable_cyst))*undercooked

Tot_formal_infective<-(Prob_light_infect_form+Prob_moder_infect_form+Prob_heavy_infect_form)
```
...
Summary(Tot_Population_inf)
```

### probability infective based on  infection intensity 
```{r}
Summary(Prob_light_infect_inform)
Summary(Prob_moder_infect_inform)
Summary(Prob_heavy_infect_inform)
Summary(Tot_informal_infective)
Summary(Prob_light_infect_form)
Summary(Prob_moder_infect_form)
Summary(Prob_heavy_infect_form)
Summary(Tot_formal_infective)
```
...
### Number of meals based on location, slaughter, pig system
```{r}
library(ggplot2)
...
### means and uncertainty intervals
means_data <- data.frame(
  group = c("Countryside", "City", "Extensive pig", "Intensive pig","Formal", "Informal", "Condemned"),
  mean = c(A_mean, B_mean, C_mean, D_mean,E_mean, F_mean, G_mean ),
  lower = c(A_lower, B_lower, C_lower, D_lower, E_lower, F_lower, G_lower),
  upper = c(A_upper, B_upper, C_upper, D_upper, E_upper, F_upper, G_upper)
)
...
ggplot(means_data, aes(x = group, y = mean, fill = group)) +
  ...
  geom_errorbar(aes(ymin = lower, ymax = upper), width = 0.2, color = "black", position = position_dodge(0.7)) +
  labs(
    x = "Description of meals",
    y = "Number of infective meals") +
  scale_fill_manual(values = c("steelblue", "steelblue", "steelblue","steelblue", "steelblue", "steelblue", "steelblue" )) +scale_y_continuous(breaks = seq(0,2000000, by =200000))+
  theme_minimal() +
  theme(panel.grid = element_blank(), legend.position = "none")
```
...
Infective_data <- data.frame(
  ...
  Prob_Formal = c(0.0019, 0.0028, 0.0039),
  CI_Formal_Lower = c(0.0007, 0.0009, 0.0013),
  CI_Formal_Upper = c(0.0039, 0.0063, 0.0084),
  Prob_informal = c(0.0009, 0.0040, 0.0059),
  CI_Informal_Lower = c(0.0004, 0.0022, 0.0030),
  CI_Informal_Upper = c(0.0016, 0.0064, 0.010)
)
...
sa_src(Tot_prob_infective_AC,
       cbind(Prop_home_countryside, Prop_home_city,Prev_MI, Prev_Tp,
             Prev_ext_MI,Prev_int_MI,  Prev_ext_pig, Prev_int_pig, Heavy_Inf, Moderate_Inf, Light_Inf, Heavy_inform, Moder_inform, Light_inform, Pmeal1, Pmeal2, Pmeal3, Viable_cyst, undercooked, meal_pig))
...
## tornado graph
Risk_AC_items <-
  c( "Prop. undercooked pork", "Prop. heavy infection informal", "Prev. tongue palpation extensive farm", "Prop. heavy infection formal", "Prop. moderate infection formal", "Prop moderate infection informal", "Prop. light infection informal", "Prop. light infection formal",  "Prob. viable cyst", "Prev. tongue palpation intensive farm",  "Prob. meal light infection", "Overall prev. meat inspection", "Overall prev. tongue palpation in pig farms","Prob. meal moderate infection", "Prev. meat inspection intensive pig","Prob. meal heavy infection", "Prev. meat inspection extensive pig", "Prop. informal slaughter city", "Prop. informal slaughter countryside", "Meals per pig")
